...
exports = module.exports = function NodeGateKeeper() {
    ...
    validator.error = function (error) {
        if (error === 'gatekeeper.error.invalid.params') {
    ...
    return {
        ...
        updateFeature: function (feature, done) {
            if (!isValidFeature(feature)) {
                ...
                done({message: 'gatekeeper.error.invalid.params', data: [
                ...
                gate.Feature.findOneAndUpdate({flag: feature.flag}, feature, function (err, result) {
                    if (!err && !result) {
                        err = {message: 'gatekeeper.error.invalid.params', data: [
            ...
            gate.permissionsFunction(req, res, function (err, result) {
                ...
                gate.isEnabled({flag: flag, permissions: result}, function (err, result) {
                    if (err) {
                        ...
                        next({message: 'gatekeeper.error.forbidden'}, result);
                    } else {
                        next(null, result);
                    }
                });
    ...
    function validateParams(value) {
        validator.check(value, 'gatekeeper.error.invalid.params').notEmpty();
        validator.check(value, 'gatekeeper.error.invalid.params').is('^[0-9a-zA-Z \'-]+$');
    }
    ...
    function isValidPermission(permission) {
        ...
        if (valid) {
            ...
            validator.errors.push({message: 'gatekeeper.error.invalid.params', data: [
