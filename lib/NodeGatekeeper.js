...
exports = module.exports = function NodeGateKeeper() {
    ...
    return {
        ...
        updateFeature: function (feature, done) {
            if (!isValidFeature(feature)) {
                ...
                done({message:'api.error.invalid.params', data:[{field:'permission', value:feature.permissions}]});
            } else {
                var gate = this;
                ...
                gate.Feature.findOneAndUpdate({flag: feature.flag}, feature, function (err, result) {
                    if(!err && !result){
                        err = {message:'api.error.invalid.params', data:[{field:'feature', value:feature}]};
                    }
                    done(err, result);
                });
            }
                ...
                gate.permissionsFunction(req, res, function (err, result) {
                    ...
                    gate.isEnabled({flag: flag, permissions: result}, function (err, result) {
                        if (err) {
                            ...
                            next({message:'api.error.forbidden'});
                        } else {
                            next();
                        }
                    });
    ...
    function isValidPermission(permission) {
        ...
        if (valid) {
            ...
            validator.errors.push({message:'api.error.invalid.params', data:[{field:'permission', value:permission}]});
        }
    ...
    function isValidFeature(feature) {
        ...
        if (valid) {
            ...
            validator.errors.push({message:'api.error.invalid.params', data:[{field:'feature', value:feature}]});
        }
        return validator.errors.length === 0;
    }
