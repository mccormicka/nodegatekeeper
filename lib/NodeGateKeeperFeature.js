...
function createSchema(connection, TYPE) {
    ...
    schema.statics.disablePermissions = function (query, permissions, done) {
        ...
        feature.findOne(query, function (err, result) {
            if (err) {
                ...
                done(new Error('gatekeeper Unable to find feature'), result);
            }
        });
    };
            ...
            if (err) {
                ...
                if (result.enabled) {
                    if (permissions) {
                        ...
                        var valid = _.any(names, function (value) {
    ...
    schema.statics.isEnabledWithoutPermission = function (query, done) {
        var feature = this;
        feature.findOne(query, function (err, result) {
            if (err) {
                done(err, null);
            } else if (result) {
                done(null, result.enabled);
            } else {
                done(null, false);
            }
        });
